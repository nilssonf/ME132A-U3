...
function findCourse () {
  let course = DATABASE.courses.filter(course =>
    course.title.toLowerCase().includes(inputCourse.value)
  )
...
function renderCourse (course) {
  ...
  for (let i = 0; i < theResponsibleTeacher.length; i++) {
    ...
    respP.innerText =
      theResponsibleTeacher[i].firstName +
      ' ' +
      theResponsibleTeacher[i].lastName +
      ' (' +
      theResponsibleTeacher[i].post +
      ')'
  }
